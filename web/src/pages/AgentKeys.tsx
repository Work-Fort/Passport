...
export default function AgentKeys() {
  ...
  const handleRevoke = async (k: ApiKey) => {
    if (!confirm(`Revoke agent key "${k.name}"?`)) return;
  ...
  const handleCreate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await createApiKey({ prefix: 'wf-agt', name, metadata: { type: 'agent' } });
      setShowCreate(false);
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h2 style={{ margin: 0, fontSize: 'var(--wf-text-lg)' }}>Agent Keys</h2>
        <button style={primaryBtn} onClick={() => setShowCreate(!showCreate)}>
          {showCreate ? 'Cancel' : '+ Create Agent Key'}
        </button>
      </div>
      ...
      {showCreate && (
        <form onSubmit={handleCreate} style={{ marginBottom: '1.5rem', padding: '1rem', border: '1px solid var(--wf-color-border)', borderRadius: 'var(--wf-radius-sm)' }}>
          <label style={{ fontSize: 'var(--wf-text-sm)' }}>Key Name<input name="name" type="text" required placeholder="e.g. claude-mcp" style={inputStyle} /></label>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
        ...
        {keys.length === 0 && <div style={{ color: 'var(--wf-color-text-secondary)', fontSize: 'var(--wf-text-sm)' }}>No agent keys. Create one for MCP bridges or AI agents.</div>}
      </div>
    </div>
  );
}
